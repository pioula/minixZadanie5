...
struct filp *filp;
{
  ...
  if (vnode->listeners_count > 0) // DEBUG
    printf("Listeners %d\n", vnode->listeners_count);

  if (mutex_lock(&vnode->cascade_lock) != 0) panic("failed notify");
  while (int i = 0; i < vnode->listeners_count; i++) {
    if (mutex_unlock(&vnode->cascade_lock) != 0) panic("failed notify");
    if (cond_signal(&vnode->for_listeners) != 0) panic("failed notify");
    if (mutex_lock(&vnode->cascade_lock) != 0) panic("failed notify");
  }
